...
class ImageDetectionRule(BaseRule):
    ...
    async def get_key(self, guild: discord.Guild):
        ...
        try:
            ...
            raise SecretKeyNotSetException("No endpoint secret key has been set, you can access this from the Azure Portal")
    ...
    async def get_endpoint(self, guild: discord.Guild):
        try:
            ...
            raise EndpointNotSetException("No endpoint URL has been set, you can access this from the Azure Portal")
    ...
    async def get_announcement_embed(
        ...
        embed = await super().get_announcement_embed(message, message_has_been_deleted, action_taken_success, action_taken, infraction_information)
    ...
    async def is_offensive(
        ...
        try:
                ...
                async with session.post(url, json=data, headers=headers) as response:
                    ...
                    if "error" in json_response:
                        if json_response['error']['code'] == '429':
                            log.warning("Being rate-limited by Azure Cognitive services. Skipping image.")
            ...
            adult_content, racy_content, gory_content = adult['isAdultContent'], adult['isRacyContent'], adult['isGoryContent']
            if adult_content or racy_content or gory_content:
                ...
                if "description" in image_details:
                    ...
                    if 'tags' in image_details['description']:
                        message += "**Tags:**\n"
                        message += ', '.join('`{0}`'.format(w) for w in image_details['description']['tags'])
